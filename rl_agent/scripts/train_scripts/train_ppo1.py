...
def train_agent_ppo(config, ns, agent_name, state_collector, total_timesteps, policy, gamma=0.99, timesteps_per_actorbatch=256,
    ...
    # Document agent
    rospy.loginfo("Starting PPO Training of agent: %s" %(agent_name))
    rospy.loginfo("------------------------------------------------------")
    rospy.loginfo("gamma \t\t\t\t %f" %model.gamma)
    rospy.loginfo("timesteps_per_actorbatch \t %d" %model.timesteps_per_actorbatch)
    rospy.loginfo("clip_param \t\t\t %f" %model.clip_param)
    rospy.loginfo("entcoeff \t\t\t %f" %model.entcoeff)
    rospy.loginfo("optim_epochs \t\t\t %d" %model.optim_epochs)
    rospy.loginfo("optim_stepsize \t\t\t %f" %model.optim_stepsize)
    rospy.loginfo("optim_batchsize \t\t %d" %model.optim_batchsize)
    rospy.loginfo("lam \t\t\t\t %f" %model.lam)
    rospy.loginfo("adam_epsilon \t\t\t %f" %model.adam_epsilon)
    rospy.loginfo("schedule \t\t\t %s" %model.schedule)
